...
export default function Index() {
  ...
  const [status, setStatus] = useState("Sin conexion (usa QR)");
  ...
  async function connectFromSaved() {
    ...
    try {
      ...
      setStatus("Guardada fallo (usa QR)");
    }
  }
  ...
  async function openQr() {
    if (!perm?.granted) {
      ...
      if (!r.granted) {
        Alert.alert("Permiso requerido", "Necesito permiso de camara para escanear QR.");
        return;
      }
    }
    setQrOpen(true);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#000", padding: 16 }}>
      <Text style={{ color: "#fff" }}>qrOpen: {String(qrOpen)}</Text>
      ...
      <View style={{ flexDirection: "row", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
        ...
        <TouchableOpacity
          onPress={() => alert("tap")}
          style={{ padding: 12, borderWidth: 1, borderColor: "#fff", borderRadius: 12 }}
        >
          <Text style={{ color: "#fff" }}>Tap test</Text>
        </TouchableOpacity>
      </View>
      ...
      >
        <SafeAreaView style={{ flex: 1, backgroundColor: "#000" }}>
          <View style={{ padding: 12, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
            <Text style={{ fontSize: 16, fontWeight: "700", color: "#fff" }}>Escanea el QR de la PC</Text>
            <Pressable
              onPress={() => setQrOpen(false)}
              style={{ padding: 10, borderWidth: 1, borderRadius: 10, borderColor: "#fff" }}
            >
